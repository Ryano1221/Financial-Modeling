...
_SYSTEM_PROMPT = (
    ...
    "- MULTI-OPTION PROPOSALS: When a document presents Option A and Option B (or multiple scenarios), "
    "extract the values for OPTION A (the first/shorter option) as the primary values. "
    "If both options share the same value (e.g. same base rent), extract that shared value. "
    "Create a review_task noting Option B differences so the user can create a second scenario.\n"
    ...
    "- tenant_improvements.allowance_psf: extract the dollar-per-SF TI budget. "
    "This includes turn-key delivery with a stated cost cap — e.g. 'turn-key...at a cost not to exceed $60.00 per sq. ft.' "
    "means allowance_psf = 60.0. 'Turn-key' with NO dollar cap should yield null (note in review_tasks).\n"
    "- abatements: 'Base Rent will be abated for the initial X months' means one abatement entry: "
