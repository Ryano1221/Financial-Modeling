...
export function SurveysWorkspace({ clientId, exportBranding = {} }: SurveysWorkspaceProps) {
  ...
  const readyCount = useMemo(() => entries.filter((entry) => !entry.needsReview).length, [entries]);
  ...
  return (
    <PlatformSection
      ...
      description="Start with one intake action, review extracted survey rows, then export a cleaner client-ready survey package."
      headerAlign="center"
      actions={
    ...
    >
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
        <div className="space-y-4 xl:col-span-8">
          <PlatformPanel kicker="Primary Action" title="Upload or select survey documents">
            <div className="space-y-4">
              <PlatformStepList
                steps={[
                  {
                    title: "Add flyers, brochures, or floorplans",
                    description: "Drop files on the Surveys tab or pull an existing client document into this workflow.",
                  },
                  {
                    title: "Confirm extracted survey rows",
                    description: "Review only the rows that need attention before exposing them to clients.",
                  },
                  {
                    title: "Export when the shortlist is ready",
                    description: "Send Excel, PDF, or a share link once the survey table is clean.",
                  },
                ]}
              />
              <div className="flex flex-col gap-3 border border-dashed border-white/20 bg-black/20 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-300">
                    Files dropped here stay in the active client library and auto-create survey rows.
                  </p>
                </div>
          ...
          <PlatformDisclosure
            kicker="Workflow"
            title="Review survey entries"
            description="Keep the table closed until you need to inspect the shortlist. Select a row to open its detail editor."
            defaultOpen
          >
          ...
          <PlatformDisclosure
            kicker="Details"
            title={selected ? `Edit ${selected.buildingName || selected.sourceDocumentName}` : "Entry editor"}
            description="Advanced fields stay tucked away until you want to refine a selected survey row."
          >
            {!selected ? (
        ...
        <div className="space-y-4 xl:col-span-4">
          <PlatformPanel kicker="Insights" title="Survey snapshot">
            <PlatformMetricStrip
              items={[
                { label: "Entries", value: entries.length },
                { label: "Ready", value: readyCount },
                { label: "Needs Review", value: reviewCount, emphasis: reviewCount > 0 },
                {
                  label: "Selected Cost",
                  value: selectedCost
                    ? new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(selectedCost.totalMonthly)
                    : "-",
                  detail: selected ? "Monthly occupancy cost" : "Select an entry",
                },
              ]}
          ...
          <PlatformPanel kicker="Map" title="Survey location map">
            <SurveyLocationsMap
              entries={entries}
          ...
          <PlatformPanel kicker="Selected Entry" title="Monthly occupancy cost">
